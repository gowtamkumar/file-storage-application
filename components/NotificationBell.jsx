...
export default function NotificationBell() {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      if (data.success) {
        ...
        const unread = data.data.filter(n => !n.readBy.includes(session.user.id)).length;
  ...
  const getTypeIcon = (type) => {
    switch (type) {
      ...
      case 'warning': return <InfoCircleOutlined className="text-orange-500" />;
      case 'error': return <InfoCircleOutlined className="text-red-500" />;
      ...
      >
        <List
          ...
          renderItem={(item) => {
            ...
            return (
              <List.Item
                className={`transition-colors duration-200 ${isRead ? 'opacity-60' : 'bg-blue-50/50'}`}
                actions={[
                  !isRead && (
              ...
              >
                <List.Item.Meta
                  ...
                  description={
                    <div>
                      <Paragraph ellipsis={{ rows: 2, expandable: true, symbol: 'more' }} className="mb-1 text-sm">
                        {item.message}
                      </Paragraph>
                    </div>
                  }
