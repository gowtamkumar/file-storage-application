...
export default function UsersPage() {
  ...
  const totalStorage = users.reduce(
    (acc, u) => acc + (u.stats?.storageUsed || 0),
    0
  );
  const totalFiles = users.reduce(
    (acc, u) => acc + (u.stats?.fileCount || 0),
    0
  );
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <div>
          <Tag
            color={getPlanColor(record.subscription?.plan)}
        ...
        <div>
          <div style={{ fontWeight: 600 }}>
            ...
            {record.subscription?.storageLimit === -1
              ? "∞"
              : `${record.subscription?.storageLimit} MB`}
          </div>
          <Progress
            percent={
              record.subscription?.storageLimit === -1
      ...
      <div style={{ maxWidth: "1400px", margin: "0 auto" }}>
        ...
        >
          <div>
            ...
            <Text type="secondary">
              Manage all users and view their statistics
            </Text>
          </div>
